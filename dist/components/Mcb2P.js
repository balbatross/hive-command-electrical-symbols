"use strict";
var __createBinding = (this && this.__createBinding) || (Object.create ? (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    var desc = Object.getOwnPropertyDescriptor(m, k);
    if (!desc || ("get" in desc ? !m.__esModule : desc.writable || desc.configurable)) {
      desc = { enumerable: true, get: function() { return m[k]; } };
    }
    Object.defineProperty(o, k2, desc);
}) : (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    o[k2] = m[k];
}));
var __setModuleDefault = (this && this.__setModuleDefault) || (Object.create ? (function(o, v) {
    Object.defineProperty(o, "default", { enumerable: true, value: v });
}) : function(o, v) {
    o["default"] = v;
});
var __importStar = (this && this.__importStar) || function (mod) {
    if (mod && mod.__esModule) return mod;
    var result = {};
    if (mod != null) for (var k in mod) if (k !== "default" && Object.prototype.hasOwnProperty.call(mod, k)) __createBinding(result, mod, k);
    __setModuleDefault(result, mod);
    return result;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.MCB2P = void 0;
const React = __importStar(require("react"));
const MCB2P = (props) => (React.createElement("svg", Object.assign({ xmlns: "http://www.w3.org/2000/svg", xmlnsXlink: "http://www.w3.org/1999/xlink", fill: "#fff", fillRule: "evenodd", stroke: "#000", strokeLinecap: "round", strokeLinejoin: "round", fontFamily: "Roboto", fontSize: 14, textAnchor: "middle", viewBox: "5.5 -4.5 30 60" }, props),
    React.createElement("defs", null,
        React.createElement("style", null, '@font-face{font-family:"Roboto";src:url(data:application/font-woff;charset=utf-8;base64,)format("woff");font-weight:400;font-style:normal}.mcb-2p_svg__B{fill:#000}.mcb-2p_svg__C{stroke-width:.5}')),
    React.createElement("use", { xlinkHref: "#mcb-2p_svg__A", x: 0.5, y: 0.5 }),
    React.createElement("symbol", { id: "mcb-2p_svg__A", overflow: "visible" },
        React.createElement("g", { fill: "none", className: "mcb-2p_svg__C" },
            React.createElement("path", { d: "M20 0v15m0 35V35l-5-20M40 0v15m0 35V35l-5-20" }),
            React.createElement("path", { strokeDasharray: "1.5 2", d: "M37.5 25h-20" })),
        React.createElement("path", { fillOpacity: 0.99, d: "M14.087 27.427 9.5 30.038l5.268.337z", className: "mcb-2p_svg__B mcb-2p_svg__C" }),
        React.createElement("path", { fill: "none", d: "m18.14 28.011-3.703.855", className: "mcb-2p_svg__C" }),
        React.createElement("path", { fillOpacity: 0.99, d: "M34.087 27.427 29.5 30.038l5.268.337z", className: "mcb-2p_svg__B mcb-2p_svg__C" }),
        React.createElement("path", { fill: "none", d: "m38.14 28.011-3.703.855m-17.756-4.827-2.768.717L13.148 22l-2.768.717.763 2.756-2.768.717m28.306-2.151-2.768.717L33.148 22l-2.768.717.763 2.756-2.768.717", className: "mcb-2p_svg__C" }),
        React.createElement("text", { className: "mcb-2p_svg__B", style: {
                stroke: "none",
                textAnchor: "end",
                fontSize: 7,
            } },
            React.createElement("tspan", { x: 18, y: 9 }, "1"),
            React.createElement("tspan", { x: 18, y: 46.81 }, "2"),
            React.createElement("tspan", { x: 38, y: 9 }, "3"),
            React.createElement("tspan", { x: 38, y: 46.81 }, "4")))));
exports.MCB2P = MCB2P;
exports.MCB2P.metadata = {
    width: 30,
    height: 60
};
