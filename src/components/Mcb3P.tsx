import * as React from "react";
import type { SVGProps } from "react";
const SvgMcb3P = (props: SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    xmlnsXlink="http://www.w3.org/1999/xlink"
    fill="#fff"
    fillRule="evenodd"
    stroke="#000"
    strokeLinecap="round"
    strokeLinejoin="round"
    fontFamily="Roboto"
    fontSize={14}
    textAnchor="middle"
    viewBox="-10 -10 91 71"
    {...props}
  >
    <defs>
      <style>
        {
          '@font-face{font-family:"Roboto";src:url(data:application/font-woff;charset=utf-8;base64,)format("woff");font-weight:400;font-style:normal}.mcb-3p_svg__B{fill:#000}.mcb-3p_svg__C{stroke-width:.5}.mcb-3p_svg__G{fill-opacity:.99}'
        }
      </style>
    </defs>
    <use xlinkHref="#mcb-3p_svg__A" x={0.5} y={0.5} />
    <symbol id="mcb-3p_svg__A" overflow="visible">
      <text
        className="mcb-3p_svg__B"
        style={{
          stroke: "none",
          textAnchor: "end",
          fontSize: 7,
        }}
      >
        <tspan x={18.002} y={9}>
          {"1"}
        </tspan>
        <tspan x={18.002} y={46.81}>
          {"2"}
        </tspan>
        <tspan x={38.001} y={9}>
          {"3"}
        </tspan>
        <tspan x={38.001} y={46.81}>
          {"4"}
        </tspan>
        <tspan x={58} y={9}>
          {"5"}
        </tspan>
        <tspan x={58} y={46.81}>
          {"6"}
        </tspan>
      </text>
      <g fill="none" className="mcb-3p_svg__C">
        <path d="M20 0v15m0 35V35l-5-20M40 0v15m0 35V35l-5-20M60 0v15m0 35V35l-5-20" />
        <path strokeDasharray="1.5 2" d="M57.5 25h-40" />
        <path d="m16.681 24.039-2.768.717L13.148 22l-2.768.717.763 2.756-2.768.717m28.306-2.151-2.768.717L33.148 22l-2.768.717.763 2.756-2.768.717m28.306-2.151-2.768.717L53.148 22l-2.768.717.763 2.756-2.768.717" />
      </g>
      <path
        d="M14.087 27.427 9.5 30.038l5.268.337z"
        className="mcb-3p_svg__B mcb-3p_svg__C mcb-3p_svg__G"
      />
      <path fill="none" d="m18.14 28.011-3.703.855" className="mcb-3p_svg__C" />
      <path
        d="M34.087 27.427 29.5 30.038l5.268.337z"
        className="mcb-3p_svg__B mcb-3p_svg__C mcb-3p_svg__G"
      />
      <path fill="none" d="m38.14 28.011-3.703.855" className="mcb-3p_svg__C" />
      <path
        d="M54.087 27.427 49.5 30.038l5.268.337z"
        className="mcb-3p_svg__B mcb-3p_svg__C mcb-3p_svg__G"
      />
      <path fill="none" d="m58.14 28.011-3.703.855" className="mcb-3p_svg__C" />
    </symbol>
  </svg>
);
export default SvgMcb3P;
