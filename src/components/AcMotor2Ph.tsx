import * as React from "react";
import type { SVGProps } from "react";
const SvgAcMotor2Ph = (props: SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    xmlnsXlink="http://www.w3.org/1999/xlink"
    fill="#fff"
    fillRule="evenodd"
    stroke="#000"
    strokeLinecap="round"
    strokeLinejoin="round"
    fontFamily="Roboto"
    fontSize={14}
    textAnchor="middle"
    viewBox="-10 -10 71 91"
    {...props}
  >
    <defs>
      <style>
        {
          '@font-face{font-family:"Roboto";src:url(data:application/font-woff;charset=utf-8;base64,)format("woff");font-weight:400;font-style:normal}.ac-motor-2ph_svg__B{fill:#000}.ac-motor-2ph_svg__C{stroke:none}.ac-motor-2ph_svg__F{stroke-width:.5}'
        }
      </style>
    </defs>
    <use xlinkHref="#ac-motor-2ph_svg__A" x={0.5} y={0.5} />
    <symbol id="ac-motor-2ph_svg__A" overflow="visible">
      <circle cx={19} cy={30} r={16} className="ac-motor-2ph_svg__F" />
      <path
        fill="none"
        d="M10 0v16.75M30 0v18.25"
        className="ac-motor-2ph_svg__F"
      />
      <path
        d="M16.669 21.863h1.055l2.024 5.133 2.024-5.133h1.059v6.291h-.817v-2.45l.077-2.644-2.033 5.094h-.625l-2.028-5.081.081 2.631v2.45h-.817v-6.291z"
        className="ac-motor-2ph_svg__B ac-motor-2ph_svg__C"
      />
      <path
        fill="none"
        d="M27.25 34.422c-4.839 7.374-4.355-5.899-9.194 1.475"
        className="ac-motor-2ph_svg__F"
      />
      <path
        d="M14.627 31.542v6.321h-.791v-5.327l-1.586.592v-.726l2.254-.86h.123z"
        className="ac-motor-2ph_svg__B ac-motor-2ph_svg__C"
      />
      <text
        className="ac-motor-2ph_svg__B ac-motor-2ph_svg__C"
        fontSize={7}
        style={{
          textAnchor: "end",
        }}
      >
        <tspan x={8} y={8.998}>
          {"1"}
        </tspan>
        <tspan x={28} y={8.998}>
          {"2"}
        </tspan>
      </text>
    </symbol>
  </svg>
);
export default SvgAcMotor2Ph;
