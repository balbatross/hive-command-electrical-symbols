import * as React from "react";
import type { SVGProps } from "react";
const SvgLiquidLevelSwitch = (props: SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    xmlnsXlink="http://www.w3.org/1999/xlink"
    fill="#fff"
    fillRule="evenodd"
    stroke="#000"
    strokeLinecap="round"
    strokeLinejoin="round"
    fontFamily="Roboto"
    fontSize={14}
    textAnchor="middle"
    viewBox="-10 -10 71 51"
    {...props}
  >
    <defs>
      <style>
        {
          '@font-face{font-family:"Roboto";src:url(data:application/font-woff;charset=utf-8;base64,)format("woff");font-weight:400;font-style:normal}.liquid-level-switch_svg__B{fill:#000}.liquid-level-switch_svg__C{stroke:none}.liquid-level-switch_svg__F{stroke-width:.5}.liquid-level-switch_svg__G{fill:none}'
        }
      </style>
    </defs>
    <use xlinkHref="#liquid-level-switch_svg__A" x={0.5} y={0.5} />
    <symbol id="liquid-level-switch_svg__A" overflow="visible">
      <path
        d="M0 10h12.5M50 10H37.5"
        className="liquid-level-switch_svg__F liquid-level-switch_svg__G"
      />
      <circle
        cx={15}
        cy={10}
        r={2.5}
        className="liquid-level-switch_svg__F liquid-level-switch_svg__G"
      />
      <circle
        cx={35}
        cy={10}
        r={2.5}
        className="liquid-level-switch_svg__F liquid-level-switch_svg__G"
      />
      <circle cx={25} cy={20} r={4} className="liquid-level-switch_svg__F" />
      <path
        d="m35 15-17.5-5M25 16v-3.75"
        className="liquid-level-switch_svg__F liquid-level-switch_svg__G"
      />
      <text
        className="liquid-level-switch_svg__B liquid-level-switch_svg__C"
        fontSize={7}
        textAnchor="start"
      >
        <tspan x={2} y={6.811}>
          {"13"}
        </tspan>
      </text>
      <text
        className="liquid-level-switch_svg__B liquid-level-switch_svg__C"
        fontSize={7}
        style={{
          textAnchor: "end",
        }}
      >
        <tspan x={48} y={6.811}>
          {"14"}
        </tspan>
      </text>
    </symbol>
  </svg>
);
export default SvgLiquidLevelSwitch;
