import * as React from "react";
import type { SVGProps } from "react";
export const FuseDisconnector3P = (props: SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    xmlnsXlink="http://www.w3.org/1999/xlink"
    fill="#fff"
    fillRule="evenodd"
    stroke="#000"
    strokeLinecap="round"
    strokeLinejoin="round"
    fontFamily="Roboto"
    fontSize={14}
    textAnchor="middle"
    viewBox="-10 -10 91 71"
    {...props}
  >
    <style>
      {
        '@font-face{font-family:"Roboto";src:url(data:application/font-woff;charset=utf-8;base64,)format("woff");font-weight:400;font-style:normal}'
      }
    </style>
    <use xlinkHref="#fuse-disconnector-3p_svg__A" x={0.5} y={0.5} />
    <symbol id="fuse-disconnector-3p_svg__A" overflow="visible">
      <g strokeWidth={0.5}>
        <path fill="none" strokeDasharray="1.5 2" d="M56 25H10" />
        <use xlinkHref="#fuse-disconnector-3p_svg__C" />
        <path fill="none" d="M20 0v15m-5 0 5 20v15M10 22.5v5" />
        <use xlinkHref="#fuse-disconnector-3p_svg__C" x={20} />
        <path fill="none" d="M40 0v15m-5 0 5 20v15" />
        <use xlinkHref="#fuse-disconnector-3p_svg__C" x={40} />
        <g fill="none">
          <path d="M60 0v15m-5 0 5 20v15M17.5 15h5m15 0h5M57.5 15h5" />
        </g>
      </g>
      <text
        style={{
          stroke: "none",
          textAnchor: "end",
          fontSize: 7,
          fill: "#000",
        }}
      >
        <tspan x={18.002} y={9}>
          {"1"}
        </tspan>
        <tspan x={18.002} y={46.81}>
          {"2"}
        </tspan>
        <tspan x={38.001} y={9}>
          {"3"}
        </tspan>
        <tspan x={38.001} y={46.81}>
          {"4"}
        </tspan>
        <tspan x={58} y={9}>
          {"5"}
        </tspan>
        <tspan x={58} y={46.81}>
          {"6"}
        </tspan>
      </text>
    </symbol>
    <defs>
      <path
        id="fuse-disconnector-3p_svg__C"
        d="m13.791 20.817 4.83-1.294 2.588 9.659-4.83 1.294z"
      />
    </defs>
  </svg>
);


FuseDisconnector3P.metadata = {
  width: 60,
  height: 60
}