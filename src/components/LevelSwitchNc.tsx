import * as React from "react";
import type { SVGProps } from "react";
const SvgLevelSwitchNc = (props: SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    xmlnsXlink="http://www.w3.org/1999/xlink"
    fill="#fff"
    fillRule="evenodd"
    stroke="#000"
    strokeLinecap="round"
    strokeLinejoin="round"
    fontFamily="Roboto"
    fontSize={14}
    textAnchor="middle"
    viewBox="-10 -10 61 71"
    {...props}
  >
    <defs>
      <style>
        {
          '@font-face{font-family:"Roboto";src:url(data:application/font-woff;charset=utf-8;base64,)format("woff");font-weight:400;font-style:normal}.level-switch-nc_svg__E{stroke-width:.5}.level-switch-nc_svg__F{fill:none}'
        }
      </style>
    </defs>
    <use xlinkHref="#level-switch-nc_svg__A" x={0.5} y={0.5} />
    <symbol id="level-switch-nc_svg__A" overflow="visible">
      <path
        d="M20 35v15M20 0v18"
        className="level-switch-nc_svg__E level-switch-nc_svg__F"
      />
      <path
        strokeDasharray="1.5 2"
        d="M23.5 25h-8"
        className="level-switch-nc_svg__E level-switch-nc_svg__F"
      />
      <path
        d="M10 22v-2"
        className="level-switch-nc_svg__E level-switch-nc_svg__F"
      />
      <path
        d="M15.534 25c-1.109 1.732-3.049 2.875-5.267 2.875S6.109 26.732 5 25c1.109-1.732 3.05-2.875 5.267-2.875s4.158 1.143 5.267 2.875z"
        className="level-switch-nc_svg__E"
      />
      <path
        d="M27.5 15 20 35M20 18h10"
        className="level-switch-nc_svg__E level-switch-nc_svg__F"
      />
      <text
        fontSize={7}
        style={{
          stroke: "none",
          fill: "#000",
        }}
        textAnchor="end"
      >
        <tspan x={18} y={9}>
          {"11"}
        </tspan>
        <tspan x={18} y={46.81}>
          {"12"}
        </tspan>
      </text>
    </symbol>
  </svg>
);
export default SvgLevelSwitchNc;
