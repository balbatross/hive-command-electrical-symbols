import * as React from "react";
import type { SVGProps } from "react";
const SvgAcMotor3Ph = (props: SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    xmlnsXlink="http://www.w3.org/1999/xlink"
    fill="#fff"
    fillRule="evenodd"
    stroke="#000"
    strokeLinecap="round"
    strokeLinejoin="round"
    fontFamily="Roboto"
    fontSize={14}
    textAnchor="middle"
    viewBox="-10 -10 101 81"
    {...props}
  >
    <defs>
      <style>
        {
          '@font-face{font-family:"Roboto";src:url(data:application/font-woff;charset=utf-8;base64,)format("woff");font-weight:400;font-style:normal}.ac-motor-3ph_svg__B{fill:#000}.ac-motor-3ph_svg__C{stroke:none}.ac-motor-3ph_svg__F{stroke-width:.5}'
        }
      </style>
    </defs>
    <use xlinkHref="#ac-motor-3ph_svg__A" x={0.5} y={0.5} />
    <symbol id="ac-motor-3ph_svg__A" overflow="visible">
      <circle cx={29} cy={30} r={16} className="ac-motor-3ph_svg__F" />
      <path
        d="M24.462 21.308h1.211l2.324 5.801 2.324-5.801h1.216v7.109H30.6v-2.769l.088-2.988-2.334 5.757h-.718l-2.329-5.742.093 2.974v2.769h-.937v-7.109z"
        className="ac-motor-3ph_svg__B ac-motor-3ph_svg__C"
      />
      <path
        fill="none"
        d="M36 35.356c-5 7.5-5-7.5-10 0"
        className="ac-motor-3ph_svg__F"
      />
      <path
        d="M21.301 35.672v-.664h.611q.576-.009.905-.303.33-.294.33-.795 0-1.125-1.121-1.125-.527 0-.842.301-.314.301-.314.798h-.813q0-.76.556-1.263.556-.503 1.413-.503.905 0 1.419.479t.514 1.331q0 .418-.27.809-.27.391-.736.584.527.167.815.554.288.387.288.945 0 .861-.562 1.367-.562.505-1.463.505t-1.466-.488Q20 37.716 20 36.916h.817q0 .505.33.809.33.303.883.303.589 0 .901-.308.312-.308.312-.883 0-.558-.343-.857-.343-.299-.989-.308H21.3z"
        className="ac-motor-3ph_svg__B ac-motor-3ph_svg__C"
      />
      <path
        fill="none"
        d="M30 0v14M49.998 0v15.319l-7.437 6.118M10 0v15.16L15.5 21"
        className="ac-motor-3ph_svg__F"
      />
      <text
        className="ac-motor-3ph_svg__B ac-motor-3ph_svg__C"
        fontSize={7}
        style={{
          textAnchor: "end",
        }}
      >
        <tspan x={8.004} y={8.998}>
          {"U1"}
        </tspan>
        <tspan x={28.004} y={8.998}>
          {"V1"}
        </tspan>
        <tspan x={48.004} y={8.998}>
          {"W1"}
        </tspan>
      </text>
    </symbol>
  </svg>
);
export default SvgAcMotor3Ph;
