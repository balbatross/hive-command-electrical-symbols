import * as React from "react";
import type { SVGProps } from "react";
const SvgLevelSwitchNo = (props: SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    xmlnsXlink="http://www.w3.org/1999/xlink"
    fill="#fff"
    fillRule="evenodd"
    stroke="#000"
    strokeLinecap="round"
    strokeLinejoin="round"
    fontFamily="Roboto"
    fontSize={14}
    textAnchor="middle"
    viewBox="-10 -10 61 71"
    {...props}
  >
    <defs>
      <style>
        {
          '@font-face{font-family:"Roboto";src:url(data:application/font-woff;charset=utf-8;base64,)format("woff");font-weight:400;font-style:normal}.level-switch-no_svg__E{stroke-width:.5}'
        }
      </style>
    </defs>
    <use xlinkHref="#level-switch-no_svg__A" x={0.5} y={0.5} />
    <symbol id="level-switch-no_svg__A" overflow="visible">
      <path
        fill="none"
        d="M20 0v15M20 35v15"
        className="level-switch-no_svg__E"
      />
      <path
        fill="none"
        strokeDasharray="1.5 2"
        d="M17.5 25h-7"
        className="level-switch-no_svg__E"
      />
      <path fill="none" d="M5.25 22v-2" className="level-switch-no_svg__E" />
      <path
        d="M10.534 25c-1.109 1.732-3.05 2.875-5.267 2.875S1.109 26.732 0 25c1.109-1.732 3.05-2.875 5.267-2.875S9.425 23.268 10.534 25z"
        className="level-switch-no_svg__E"
      />
      <path fill="none" d="m15 15 5 20" className="level-switch-no_svg__E" />
      <text
        fontSize={7}
        style={{
          stroke: "none",
          fill: "#000",
        }}
        textAnchor="end"
      >
        <tspan x={18} y={9}>
          {"13"}
        </tspan>
        <tspan x={18} y={46.81}>
          {"14"}
        </tspan>
      </text>
    </symbol>
  </svg>
);
export default SvgLevelSwitchNo;
