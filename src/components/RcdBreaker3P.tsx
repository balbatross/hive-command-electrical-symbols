import * as React from "react";
import type { SVGProps } from "react";
export const RcdBreaker3P = (props: SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    xmlnsXlink="http://www.w3.org/1999/xlink"
    fill="#fff"
    fillRule="evenodd"
    stroke="#000"
    strokeLinecap="round"
    strokeLinejoin="round"
    fontFamily="Roboto"
    fontSize={14}
    textAnchor="middle"
    viewBox="-10 -10 91 71"
    {...props}
  >
    <defs>
      <style>
        {
          '@font-face{font-family:"Roboto";src:url(data:application/font-woff;charset=utf-8;base64,)format("woff");font-weight:400;font-style:normal}.rcd-breaker-3p_svg__B{stroke-width:.5}.rcd-breaker-3p_svg__C{fill:#000}.rcd-breaker-3p_svg__G{fill-opacity:.99}'
        }
      </style>
    </defs>
    <use xlinkHref="#rcd-breaker-3p_svg__A" x={0.5} y={0.5} />
    <symbol id="rcd-breaker-3p_svg__A" overflow="visible">
      <path
        fill="none"
        d="M60 0v15m0 35V35l-5-20"
        className="rcd-breaker-3p_svg__B"
      />
      <path
        d="m53.297 24.446-4.587 2.611 5.268.337z"
        className="rcd-breaker-3p_svg__B rcd-breaker-3p_svg__C rcd-breaker-3p_svg__G"
      />
      <path
        fill="none"
        d="m57.351 25.03-3.703.855M40 0v15m0 35V35l-5-20"
        className="rcd-breaker-3p_svg__B"
      />
      <path
        d="m33.297 24.446-4.587 2.611 5.268.337z"
        className="rcd-breaker-3p_svg__B rcd-breaker-3p_svg__C rcd-breaker-3p_svg__G"
      />
      <g fill="none">
        <path
          d="m37.351 25.03-3.703.855m-5.713 3.085-1.103-3.845M24.5 27.874l2.884-.827m20.518 2.203-1.103-3.845M44.467 28.154l2.884-.827M20 0v15m0 35V35l-5-20"
          className="rcd-breaker-3p_svg__B"
        />
      </g>
      <path
        d="M13.297 24.446 8.71 27.057l5.268.337z"
        className="rcd-breaker-3p_svg__B rcd-breaker-3p_svg__C rcd-breaker-3p_svg__G"
      />
      <g fill="none">
        <path
          d="m17.351 25.03-3.703.855M7.935 28.97l-1.103-3.845M4.5 27.874l2.884-.827"
          className="rcd-breaker-3p_svg__B"
        />
        <path
          strokeDasharray="1.5 2"
          d="M57.402 25h-40.5"
          className="rcd-breaker-3p_svg__B"
        />
      </g>
      <text
        className="rcd-breaker-3p_svg__C"
        style={{
          stroke: "none",
          textAnchor: "end",
          fontSize: 7,
        }}
      >
        <tspan x={18.002} y={9}>
          {"1"}
        </tspan>
        <tspan x={18.002} y={46.81}>
          {"2"}
        </tspan>
        <tspan x={38.001} y={9}>
          {"3"}
        </tspan>
        <tspan x={38.001} y={46.81}>
          {"4"}
        </tspan>
        <tspan x={58} y={9}>
          {"5"}
        </tspan>
        <tspan x={58} y={46.81}>
          {"6"}
        </tspan>
      </text>
    </symbol>
  </svg>
);

RcdBreaker3P.metadata = {
  width: 60,
  height: 60
}